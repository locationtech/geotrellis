...
class SpatialTemporalGeoTiffAdapterSpec extends TestEnvironment with Matchers {
  lazy val uri: String = "src/test/resources/raster/all-ones.tif"

  describe("SpatialTemporalGeoTiffAdapterSpec spatial temporal index spec") {
    // This describes how to index the data
    val index = new SpatialTemporalIndexBuilder().createIndex

    // Register our adapter in a datastore
    lazy val geowaveDataStore: DataStore = DataStoreFactory.createDataStore(new CassandraRequiredOptions(cassandra, "SpatialTemporalGeoTiffAdapterSpec", new CassandraOptions()))
    lazy val tiff = MultibandGeoTiff(uri)
    lazy val dataTypeAdapter = new GeoTiffAdapter("SpatialTemporalGeoTiffAdapterSpec".typeName)
    geowaveDataStore.addType(dataTypeAdapter, index)

    it("should write spatially temporal indexed GeoTiff") {
    ...
    it("should return nothing querying out of bounds") {
      val extent = Extent(0, 0, 1, 1)
      val dateString = "2017:11:01 12:00:00"
      val date = Date.from(ZonedDateTime.from(GEOTIFF_TIME_FORMATTER_DEFAULT.parse(dateString)).toInstant)
      val spatialTemporalQuery = new ExplicitSpatialTemporalQuery(date, date, extent.toPolygon())
