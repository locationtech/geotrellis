...
/** In memory data store is not threadsafe, for these reasons it is ignored, but it is a pretty convenient for local development. */
class IMMultibandRasterAdapterSpec extends AnyFunSpec with Matchers {
  lazy val uri: String = "src/test/resources/raster/all-ones.tif"

  /** To run this test change "ignore" to "describe" */
  ignore("In Memory MultibandRasterAdapterSpec spatial index spec") {
    // This describes how to index the data
    val index = new SpatialIndexBuilder().createIndex

    // Register our adapter in a datastore
    lazy val geowaveDataStore: DataStore = DataStoreFactory.createDataStore(new MemoryRequiredOptions)
    lazy val raster = MultibandGeoTiff(uri).raster
    lazy val dataTypeAdapter = new MulitbandRasterAdapter("MultibandRasterDataAdapterSpec".typeName)
  ...
  describe("MultibandRasterAdapterSpec serialization spec") {
    it("should be serialized properly") {
      val expected = new MulitbandRasterAdapter("MultibandRasterDataAdapterSpec".typeName)
