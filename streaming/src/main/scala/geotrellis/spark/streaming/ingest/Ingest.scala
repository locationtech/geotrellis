...
import geotrellis.spark.ingest.{Ingest => RDDIngest}
  ...
  def apply[T: ClassTag: ? => TilerKeyMethods[T, K]: Component[?, ProjectedExtent], K: SpatialComponent: Boundable: ClassTag](
    ...
    (sink: (TileLayerRDD[K], Int) => Unit): Unit =
      sourceTiles
        .foreachRDD { rdd =>
          if(!rdd.isEmpty)
            RDDIngest[T, K](rdd, destCRS, layoutScheme, pyramid, cacheLevel, resampleMethod, partitioner, bufferSize)(sink)
        }
}
