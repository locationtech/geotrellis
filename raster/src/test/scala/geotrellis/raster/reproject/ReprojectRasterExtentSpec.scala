  ...
  describe("ReprojectRasterExtent") {
    ...
    it("should (approximately) match a GDAL for EPSG:4326 to EPSG:3857") {
      ...
      val rea @ RasterExtent(actualExtent, actualCellWidth, actualCellHeight, actualCols, actualRows) =
        ReprojectRasterExtent(sourceRaster.rasterExtent, sourceGt.crs, WebMercator, IdentityResampleTarget)
    ...
    it("should be in approximation to GDAL EPSG:32618 to EPSG:3857") {
      ...
      val rea @ RasterExtent(actualExtent, actualCellWidth, actualCellHeight, actualCols, actualRows) = ReprojectRasterExtent(rasterExtent, src, dest, IdentityResampleTarget)
    ...
    it("should be in approximation to GDAL EPSG:32618 to EPSG:4326") {
      ...
      val rea @ RasterExtent(actualExtent, _, _, actualCols, actualRows) = ReprojectRasterExtent(rasterExtent, src, dest, IdentityResampleTarget)
    ...
    it("should have an extent that tightly covers the polygon reprojection of the source extent") {
      ...
      val destinationRE = ReprojectRasterExtent(originalRE, crs, WebMercator, IdentityResampleTarget)
