...
class NearestNeighborResampleSpec extends FunSpec with Matchers with TestFiles {
  ...
  describe("it should correctly resample using NearestNeighbor") {
    ...
    it("should noop resample") {
      val dst = src
      val rr = resample(tile, e, TargetGridExtent(dst))
      assert(rr === tile)
    }
    ...
    it("should resize quad8") {
      ...
      val resize1 = r.resample(src, TargetGridExtent(re))

      val resize2 = r.resample(src, TargetGridExtent(re.withDimensions(40, 40)))
    ...
    it("should resize quad8 to 4x4") {
      ...
      val raster = loadTestArg("quad8").tile.resample(re.extent, TargetGridExtent(re.withDimensions(4, 4)))
