  ...
  protected def convertToConstantNoData(cellType: DataType with ConstantNoData): Array[Byte] =
    cellType match {
      case ByteConstantNoDataCellType =>
        ...
        cfor(0)(_ < size, _ + 1) { i => arr(i) = udd2b(get(i)) }
        arr
      case UByteConstantNoDataCellType =>
        val arr = Array.ofDim[Byte](size)
        cfor(0)(_ < size, _ + 1) { i => arr(i) = udd2ub(get(i)) }
        arr
      case ShortConstantNoDataCellType =>
        val arr = Array.ofDim[Short](size)
        cfor(0)(_ < size, _ + 1) { i => arr(i) = udd2s(get(i)) }
        arr.toArrayByte()
      case UShortConstantNoDataCellType =>
        val arr = Array.ofDim[Short](size)
        cfor(0)(_ < size, _ + 1) { i => arr(i) = udd2us(get(i)) }
        arr.toArrayByte()
      ...
      case FloatConstantNoDataCellType =>
        ...
        cfor(0)(_ < size, _ + 1) { i => arr(i) = udd2f(get(i)) }
        arr.toArrayByte()
      case DoubleConstantNoDataCellType =>
        val arr = Array.ofDim[Double](size)
        cfor(0)(_ < size, _ + 1) { i => arr(i) = udd2d(get(i)) }
        arr.toArrayByte()
    }
  ...
  protected def convertToUserDefinedNoData(cellType: DataType with UserDefinedNoData[_]): Array[Byte] =
    cellType match {
      case ByteUserDefinedNoDataCellType(nd) =>
        ...
        cfor(0)(_ < size, _ + 1) { i =>
          val v = get(i)
          arr(i) = if (v == Double.NaN) nd else v.toByte
        }
        arr
      ...
      case ShortUserDefinedNoDataCellType(nd) =>
        ...
        cfor(0)(_ < size, _ + 1) { i =>
          val v = get(i)
          arr(i) = if (v == Double.NaN) nd else v.toShort
        }
        arr.toArrayByte()
      ...
      case IntUserDefinedNoDataCellType(nd) =>
        ...
        cfor(0)(_ < size, _ + 1) { i =>
          val v = get(i)
          arr(i) = if (v == Double.NaN) nd else v.toInt
        }
        arr.toArrayByte()
      ...
      case DoubleUserDefinedNoDataCellType(nd) =>
        ...
        cfor(0)(_ < size, _ + 1) { i =>
          val v = get(i)
          arr(i) = if (v == Double.NaN) nd else v.toDouble
        }
        arr.toArrayByte()
    }
}
