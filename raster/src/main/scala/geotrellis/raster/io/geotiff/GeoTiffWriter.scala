...
object GeoTiffWriter {
  ...
  def write(path: String, tile: Tile, extent: Extent, nodata: Double) {
    Encoder.writePath(path, tile, RasterExtent(extent, tile.cols, tile.rows), settings(tile.cellType).setNodata(nodata))   
  }
