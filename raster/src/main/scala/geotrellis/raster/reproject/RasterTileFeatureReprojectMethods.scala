...
abstract class RasterTileFeatureReprojectMethods[
  ...
  def reproject[N: Integral](transform: Transform, inverseTransform: Transform, resampleTarget: Option[ResampleTarget]): TileFeature[Raster[T], D] =
    TileFeature(self.tile.tile.reproject(self.tile.extent, transform, inverseTransform, resampleTarget), self.data)

  def reproject[N: Integral](src: CRS, dest: CRS, resampleTarget: Option[ResampleTarget]): TileFeature[Raster[T], D] =
