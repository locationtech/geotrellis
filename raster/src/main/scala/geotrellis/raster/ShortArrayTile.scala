...
object ShortArrayTile {
  ...
  def empty(cols: Int, rows: Int, cellType: ShortCells with NoDataHandling): ShortArrayTile =
    cellType match {
      ...
      case udct @ ShortUserDefinedNoDataCellType(userNoDataValue) =>
        new ShortUserDefinedNoDataArrayTile(Array.ofDim[Short](cols * rows).fill(userNoDataValue), cols, rows, udct)
    }
