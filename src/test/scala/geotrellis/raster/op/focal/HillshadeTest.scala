...
class HillshadeTest extends FunSuite {
  ...
  test("can write hillshade") {
    ...
    val t2 = time()
    server.run(io.WritePng(r1, "/tmp/hillshade2.png", grayBreaks, h, 0))
    ...
    val t3 = time()
    server.run(io.WritePng(r, "/tmp/raster3.png", stat.GetColorBreaks(h, colors10), h, 0))
    println("[3] wrote png in %s ms" format (time() - t3))

    val t4 = time()
    server.run(io.WritePng(r, "/tmp/raster4.png", stat.GetColorBreaks(h, colors100), h, 0))
    println("[4] wrote png in %s ms" format (time() - t4))

    val t5 = time()
    server.run(io.WritePng(r, "/tmp/raster5.png", stat.GetColorBreaks(h, colors1000), h, 0))
