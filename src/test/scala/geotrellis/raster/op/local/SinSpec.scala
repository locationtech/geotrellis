...
class SinSpec extends FunSpec
  ...
  describe("Sin") {
    it("finds the sine of a double raster") {
      ...
      run(rs.localSin) match {
        case Complete(result, success) =>
          for (y <- 0 until 6) {
            for (x <- 0 until 6) {
              val theSin = result.getDouble(x, y)
              val epsilon = math.ulp(theSin)
              val theExpected = expected(y*6 + x)
              if (x >= 3 && y == 5) {
                theSin.isNaN should be (true)
              } else {
                theSin should be (theExpected plusOrMinus epsilon)
              }
            }
          }
    ...
    it("finds the sine of an int raster") {
      val rasterData = Array (
        0, 1,   2, 3,
        4, 5,   6, 7,
