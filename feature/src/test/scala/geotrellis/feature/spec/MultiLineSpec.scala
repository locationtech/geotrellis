...
class MultiLineSpec extends FunSpec with ShouldMatchers {
  ...
  describe("MultiLine") {
    ...
    it("should have a non-empty boundary if it is not closed") {
      ...
      ml.boundary should be (MultiPointResult(Set(Point(1,1), Point(2,2), Point(5,5), Point(6,10))))
    }
    ...
    it("should intersect with a MultiPolygon and return a MultiPointResult") {
      ...
      val ml = MultiLine(l1,l2)
      val mp = MultiPolygon(p1, p2)
      ml & mp should be (MultiPointResult(Set(Point(3,0), Point(3,5))))
    }
    ...
    it("should intersect with a MultiPolygon and return a MultiLineResult") {
      ...
      val ml = MultiLine(l1,l2)
      val mp = MultiPolygon(p1, p2)
      ml & mp should be (MultiLineResult(Set(Line(Point(3,0), Point(6,0)), Line(Point(3,8), Point(6,8)))))
    }
    ...
    it("should intersect with a MultiPolygon and return a GeometryCollectionResult") {
      ...
      val expected: GeometryCollection =
        GeometryCollection(points = Set(Point(6, 8)), lines = Set(Line(Point(3, 0), Point(6, 0))))
      val result = ml & mp
      result match {
    ...
    it("should union with a MultiLine and return a NoResult") {
      val ml1 = MultiLine(Set())
      val ml2 = MultiLine(Set())
      ml1 | ml2 should be (NoResult)
    }

    it("should union with a MultiLine and return a LineResult") {
      val ml1 = MultiLine(Set(Line(Point(1,1), Point(5,5))))
      val ml2 = MultiLine(Set(Line(Point(1,1), Point(5,5))))
      ml1 | ml2 should be (LineResult(Line(Point(1,1), Point(5,5))))
    }

    it("should union with a MultiLine and return a MultiLineResult") {
      val ml1 = MultiLine(Set(Line(Point(1,1), Point(5,5))))
      val ml2 = MultiLine(Set(Line(Point(1,1), Point(1,5))))
      ml1 | ml2 should be (MultiLineResult(Set(Line(Point(1,1), Point(5,5)), Line(Point(1,1), Point(1,5)))))
    }
    ...
    it("should union with an empty MultiPolygon and return a MultiLineResult") {
      ...
      val mp = MultiPolygon(Set())
      ml | mp should be (MultiLineResult(Set(Line(Point(1,1), Point(5,5)))))
    }

    it("should union with a MultiPolygon and return a PolygonResult") {
      val ml = MultiLine(Set(Line(Point(0,0), Point(2,2))))
    ...
    it("should union with a MultiPolygon and return a MultiPolygonResult") {
      ...
      val mp = MultiPolygon(p1, p2)
      ml | mp should be (MultiPolygonResult(Set(p1, p2)))
    }

    it("should union with a MultiPolygon and return a GeometryCollectionResult") {
      val ml = MultiLine(Set(Line(Point(1,11), Point(1,21))))
      val p1 = Polygon(Line(Point(0,0), Point(0,2), Point(2,2), Point(2,0), Point(0,0)))
      val p2 = Polygon(Line(Point(4,4), Point(4,6), Point(6,6), Point(6,4), Point(4,4)))
      val mp = MultiPolygon(p1, p2)
      val expected: GeometryCollection =
        GeometryCollection(lines = Set(Line(Point(1,11), Point(1,21))), polygons = Set(p1, p2))
      val result = ml | mp
      result match {
        case GeometryCollectionResult(gc) => gc should be (expected)
        case _ => fail()
    ...
    it("should difference with a MultiPoint and return a NoResult") {
      val ml = MultiLine(Set())
      val mp = MultiPoint(Set(Point(1,1), Point(2,2)))
      ml - mp should be (NoResult)
    }
    ...
    it("should difference with a MultiPolygon and return a NoResult") {
      val ml = MultiLine(Set(Line(Point(1,1), Point(5,5))))
      val mp = MultiPolygon(Set(Polygon(Line(Point(0,0), Point(0,10), Point(10,10), Point(10, 0), Point(0,0)))))
      ml - mp should be (NoResult)
    }
    ...
    it("should difference with a Point and return a MultiLineResult") {
      val ml = MultiLine(Set(Line(Point(1,1), Point(5,5)), Line(Point(0,0), Point(0, 5))))
      val p = Point(1,1)
      ml - p should be (MultiLineResult(Set(Line(Point(1,1), Point(5,5)), Line(Point(0,0), Point(0, 5)))))
    }

    it("should difference with a MultiPoint and return a MultiLineResult") {
      val ml = MultiLine(Set(Line(Point(1,1), Point(5,5)), Line(Point(0,0), Point(0, 5))))
      val mp = MultiPoint(Set(Point(1,1), Point(5,5)))
      ml - mp should be (MultiLineResult(Set(Line(Point(1,1), Point(5,5)), Line(Point(0,0), Point(0, 5)))))
    }

    it("should difference with a Line and return a MultiLineResult") {
      val ml = MultiLine(Set(Line(Point(1,1), Point(5,5)), Line(Point(0,0), Point(0,10))))
      val l = Line(Point(10,10), Point(50,50))
      ml - l should be (MultiLineResult(Set(Line(Point(1,1), Point(5,5)), Line(Point(0,0), Point(0,10)))))
    }

    it("should difference with a MultiLine and return a MultiLineResult") {
      val ml1 = MultiLine(Set(Line(Point(1,1), Point(5,5)), Line(Point(0,0), Point(0,10))))
      val ml2 = MultiLine(Set())
      ml1 - ml2 should be (MultiLineResult(Set(Line(Point(1,1), Point(5,5)), Line(Point(0,0), Point(0,10)))))
    }

    it("should difference with a Polygon and return a MultiLineResult") {
      val ml = MultiLine(Set(Line(Point(2,2), Point(5,5)), Line(Point(20,20), Point(20,30))))
      val p = Polygon(Line(Point(0,0), Point(0,1), Point(1,1), Point(1, 0), Point(0,0)))
      ml - p should be (MultiLineResult(Set(Line(Point(2,2), Point(5,5)), Line(Point(20,20), Point(20,30)))))
    }

    it("should difference with a MultiPolygon and return a MultiLineResult") {
      val ml = MultiLine(Set(Line(Point(1,1), Point(5,5)), Line(Point(20,20), Point(20,30))))
      val mp = MultiPolygon(Set())
      ml - mp should be (MultiLineResult(Set(Line(Point(1,1), Point(5,5)), Line(Point(20,20), Point(20,30)))))
    }
    ...
    it("should symDifference with a MultiLine and return a LineResult") {
      val ml1 = MultiLine(Set(Line(Point(3,3), Point(5,5))))
      val ml2 = MultiLine(Set(Line(Point(1,1), Point(5,5))))
    ...
    it("should symDifference with an empty MultiPolygon and return a MultiLineResult") {
      val ml = MultiLine(Set(Line(Point(1,1), Point(5,5))))
      val mp = MultiPolygon(Set())
      ml.symDifference(mp) should be (MultiLineResult(Set(Line(Point(1,1), Point(5,5)))))
    }

    it("should symDifference with an empty MultiPolygon and return a MultiLineResult " +
        "when there are multiple lines in the set") {
      val ml = MultiLine(Set(Line(Point(1,1), Point(5,5)), Line(Point(0,0), Point(0,10))))
      val mp = MultiPolygon(Set())
      ml.symDifference(mp) should be (MultiLineResult(Set(Line(Point(1,1), Point(5,5)), Line(Point(0,0), Point(0,10)))))
    }
    ...
    it("should symDifference with a MultiPolygon and return a MultiPolygonResult") {
      ...
      ml.symDifference(mp) should be (MultiPolygonResult(Set(p1, p2)))
    }

    it("should symDifference with a MultiPolygon and return a GeometryCollectionResult") {
      val ml = MultiLine(Set(Line(Point(1,11), Point(5,51))))
      val mp = MultiPolygon(Set(Polygon(Line(Point(0,0), Point(0,10), Point(10,10), Point(10, 0), Point(0,0)))))
      val expected: GeometryCollection =
        GeometryCollection(lines = Set(Line(Point(1,11), Point(5,51))),
            polygons = Set(Polygon(Line(Point(0,0), Point(0,10), Point(10,10), Point(10, 0), Point(0,0)))))
    ...
    it("should contain a Point") {
      val ml = MultiLine(Set(Line(Point(0,0), Point(0,10)), Line(Point(0,0), Point(10,0))))
      val p = Point(5,0)
    ...
    it("should contain a MultiPoint") {
      val ml = MultiLine(Set(Line(Point(0,0), Point(0,10)), Line(Point(0,0), Point(10,0))))
      val mp = MultiPoint(Set(Point(5,0), Point(0,8)))
    ...
    it("should contain a MultiLine") {
      val ml1 = MultiLine(Set(Line(Point(0,0), Point(0,10)), Line(Point(0,0), Point(10,0))))
      val ml2 = MultiLine(Set(Line(Point(4,0), Point(6,0)), Line(Point(0,0), Point(0,5))))
      ml1.contains(ml2) should be (true)
    }

    it("should be covered by a line") {
      val ml = MultiLine(Set(Line(Point(1,1), Point(3,3)), Line(Point(5,5), Point(7,7))))
      val l = Line(Point(0,0), Point(10,10))
      ml.coveredBy(l) should be (true)
    }

    it("should be covered by a MultiLine") {
      val ml1 = MultiLine(Set(Line(Point(1,1), Point(3,3)), Line(Point(5,5), Point(7,7))))
      val ml2 = MultiLine(Set(Line(Point(0,0), Point(10,10))))
    ...
    it("should cross a MultiPoint") {
      ...
      val ml = MultiLine(Set(Line(Point(3,3), Point(6,6))))
    ...
    it("should cross a MultiLine") {
      val ml1 = MultiLine(Set(Line(Point(0,0), Point(0,10)), Line(Point(0,0), Point(10,0))))
      val ml2 = MultiLine(Set(Line(Point(5,5), Point(5,-5))))
    ...
    it("should cross a MultiPolygon") {
      val ml = MultiLine(Set(Line(Point(0,0), Point(0,10)), Line(Point(0,0), Point(10,0))))
      val mp = MultiPolygon(Set(Polygon(Line(Point(5,5), Point(5,-5), Point(7,-5), Point(7,7), Point(5,5)))))
      ml.crosses(mp) should be (true)
    }

    it("should overlap a MultiLine") {
      val ml1 = MultiLine(Set(Line(Point(0,0), Point(5,5))))
      val ml2 = MultiLine(Set(Line(Point(3,3), Point(10,10))))
    ...
    it("should touch a MultiPoint") {
      val ml = MultiLine(Set(Line(Point(0,0), Point(0,10)), Line(Point(0,0), Point(10,0))))
      val mp = MultiPoint(Set(Point(10,0), Point(0,10)))
    ...
    it("should touch a MultiLine") {
      val ml1 = MultiLine(Set(Line(Point(0,0), Point(0,10)), Line(Point(0,0), Point(10,0))))
      val ml2 = MultiLine(Set(Line(Point(10,0), Point(10,5))))
    ...
    it("should touch a MultiPolygon") {
      val ml = MultiLine(Set(Line(Point(0,0), Point(0,10)), Line(Point(0,0), Point(10,0))))
      val mp = MultiPolygon(Set(Polygon(Line(Point(10,0), Point(10,5), Point(12,5), Point(12,0), Point(10,0)))))
      ml.touches(mp) should be (true)
    }

    it("should be within a Line") {
      val ml = MultiLine(Set(Line(Point(3,3), Point(5,5)), Line(Point(6,6), Point(8,8))))
      val l = Line(Point(0,0), Point(10,10))
      ml.within(l) should be (true)
    }

    it("should be within a MultiLine") {
      val ml1 = MultiLine(Set(Line(Point(3,3), Point(5,5)), Line(Point(6,6), Point(8,8))))
      val ml2 = MultiLine(Set(Line(Point(0,0), Point(10,10))))
    ...
    it("should be within a MultiPolygon") {
      val ml = MultiLine(Set(Line(Point(3,3), Point(5,5)), Line(Point(6,6), Point(8,8))))
      val p = MultiPolygon(Set(Polygon(Line(Point(0,0), Point(0,10), Point(10,10), Point(10,0), Point(0,0)))))
      ml.within(p) should be (true)
    }
