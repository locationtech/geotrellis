...
class WKTParserTest extends FunSpec {
  ...
  it("Should return Unit object") {
    import WKTParser._
    val expected = new Unit("degree", 0.017453292519943295, None)
    parseAll(unit, """UNIT["degree", 0.017453292519943295]""") match {
      case Success(n, _) =>
        assert(n == expected)
      case Failure(msg, _) =>
        info(msg)
  ...
  it("Should return Geogcs object") {
    ...
    val unit = new Unit("degree", 0.017453292519943295, None)
    val axisOne = new Axis("Geodetic longitude", "EAST")
    val axisTwo = new Axis("Geodetic latitude", "NORTH")
    val expected = new Geogcs("Australian Antarctic (3D deg)", datum, primeM, unit, Some(List(axisOne, axisTwo)), Some(auth))
  ...
  it("Should return a Projcs objects") {
    ...
    val primeMAuth = new Authority("EPSG","8901")
    val primeM = new PrimeM("Greenwich", 0.0, Some(primeMAuth))
    val geogcsUnit = new Unit("degree", 0.017453292519943295, None)
    ...
    val expected = new Projcs("RGF93 / CC42", geogcs, projection, Some(paramsList), unit, Some(twins), Some(projAuth))
  ...
  it("Should return VertCS object") {
    ...
    val unit = new Unit("m", 1.0, None)
    val expected = new VertCS("EVRF2007 height", vertDatum, unit, Some(axis), Some(vertAuth))
  ...
  it("Should return a LocalCS object") {
    ...
    val unit = new Unit("m", 1.0, None)
    val expected = new LocalCS("Tombak LNG plant", localDatum, unit, axisList, Some(localAuth))
  ...
  it("Should return a Geoccs object") {
    ...
    val expected = new Geoccs("REGCAN95", datum, primeM, unit, Some(List(axis1, axis2, axis3)), Some(auth))
  ...
  it("Should return a Compdcs object") {
    ...
    val geogcs = new Geogcs("ETRS89", datum, primeM, unit, Some(List(axis1, axis2)), Some(new Authority("EPSG","4258")))
    ...
    val projcs = new Projcs("ETRS89 / UTM zone 36N", geogcs, projection, Some(paramsList), new Unit("m", 1.0, None), Some(twinsProj), Some(authProj))
    ...
    val vertUnit = new Unit("m", 1.0, None)
  ...
  it("Should return a Projcs object special one") {
    ...
    val geogcs = new Geogcs("Camacupa", datum, primeM, unit, Some(List(axis1, axis2)), Some(authGeogcs))
    val projection = new Projection("Transverse_Mercator", Some(new Authority("EPSG","9807")))
    ...
    val paramsList = List(param1, param2, param3, param4, param5)
    val unitProj = new Unit("m", 1.0, None)
    val axisProj1 = new Axis("Easting", "EAST")
    val axisProj2 = new Axis("Northing", "NORTH")
  ...
  it("Should return a contain the Geoccs object") {
    ...
    val auth = new Authority("EPSG","4079")
    val expected = new Geoccs("REGCAN95", datum, primeM, unit, Some(List(axis1, axis2, axis3)), Some(auth))
    assert(WKTParser.containsObject(expected))
  }
