...
class AverageReprojectSpec extends FunSpec
    with TileBuilders
    with TestEngine {

  describe("reprojects tile using average in approximation to GDAL") {

    it ("should (approximately) match a GDAL average interpolation on nlcd webmercator tile") {
    ...
    it("should (approximately) match a GDAL average on nlcd lat lon tile") {
      val GeoTiffBand(source, extent, crs, _) = GeoTiffReader
        .read("raster-test/data/reproject/nlcd_tile_wsg84.tif")
        .firstBand
    ...
    it("should (approximately) match a GDAL average on nlcd utm tile") {
      val GeoTiffBand(source, extent, crs, _) = GeoTiffReader
        .read("raster-test/data/reproject/nlcd_tile_wsg84.tif")
        .firstBand
