...
class CubicSplineReprojectSpec extends FunSpec
    with TileBuilders
    with TestEngine {

  describe("reprojects tile using cubic spline in approximation to GDAL") {

    it ("should (approximately) match a GDAL cubic spline interpolation on nlcd webmercator tile") {
    ...
    it("should (approximately) match a GDAL cubic spline on nlcd lat lon tile") {
      val GeoTiffBand(source, extent, crs, _) = GeoTiffReader
        .read("raster-test/data/reproject/nlcd_tile_wsg84.tif")
        .firstBand
    ...
    it("should (approximately) match a GDAL cubic spline on nlcd utm tile") {
      val GeoTiffBand(source, extent, crs, _) = GeoTiffReader
        .read("raster-test/data/reproject/nlcd_tile_wsg84.tif")
        .firstBand
