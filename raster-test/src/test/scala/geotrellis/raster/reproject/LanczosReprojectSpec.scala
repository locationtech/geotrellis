...
class LanczosReprojectSpec extends FunSpec
    with TileBuilders
    with TestEngine {

  describe("reprojects tile using lanczos in approximation to GDAL") {

    it ("should (approximately) match a GDAL lanczos interpolation on nlcd webmercator tile") {
    ...
    it("should (approximately) match a GDAL lanczos on nlcd lat lon tile") {
      val GeoTiffBand(source, extent, crs, _) = GeoTiffReader
        .read("raster-test/data/reproject/nlcd_tile_wsg84.tif")
        .firstBand
    ...
    it("should (approximately) match a GDAL lanczos on nlcd utm tile") {
      val GeoTiffBand(source, extent, crs, _) = GeoTiffReader
        .read("raster-test/data/reproject/nlcd_tile_wsg84.tif")
        .firstBand
