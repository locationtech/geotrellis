...
class MultiBandGeoTiffReaderSpec extends FunSpec 
  ...
  describe("Reading geotiffs with INTERLEAVE=PIXEL") {
    ...
    ignore("COMPRESSION=DEFLATE, Stripped") {
      val tiles =
        GeoTiff(s"$filePath/geotiff-reader-tiffs/3bands/3bands-deflate.tif").bands.map(_.tile).toArray
  ...
  describe("Reading geotiffs with INTERLEAVE=BANDS") {
    ignore("Uncompressed, Stripped") {
      val tiles =
        GeoTiff(s"$filePath/geotiff-reader-tiffs/3bands/3bands-interleave-bands.tif").bands.map(_.tile).toArray
    ...
    ignore("Uncompressed, Tiled") {
      val tiles =
        GeoTiff(s"$filePath/geotiff-reader-tiffs/3bands/3bands-tiled-interleave-bands.tif").bands.map(_.tile).toArray
    ...
    ignore("COMPRESSION=DEFLATE, Stripped") {
      val tiles =
        GeoTiff(s"$filePath/geotiff-reader-tiffs/3bands/3bands-interleave-bands-deflate.tif").bands.map(_.tile).toArray
    ...
    ignore("COMPRESSION=DEFLATE, Tiled") {
      val tiles =
        GeoTiff(s"$filePath/geotiff-reader-tiffs/3bands/3bands-tiled-interleave-bands-deflate.tif").bands.map(_.tile).toArray
