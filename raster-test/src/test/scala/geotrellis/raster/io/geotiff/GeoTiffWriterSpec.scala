...
class GeoTiffWriterSpec extends FunSpec
  ...
  describe ("writing GeoTiffs without errors") {

    it ("should write floating point rasters without errors") {
      ...
      GeoTiffWriter.write(path, tile, e, crs)
  ...
  describe ("writing and then reading GeoTiffs, yielding same Proj4 Strings") {

    it ("should write GeoTiff with testProj4String, and then reading would give same string") {
      val e = Extent(100.0, 400.0, 120.0, 420.0)
      val tile = DoubleArrayTile(Array(11.0, 22.0, 33.0, 44.0), 2, 2)
      val crs = testCRS
