...
class GeoTiffWriterSpec2 extends FunSpec
    with Matchers
...
//       GeoTiffWriter2.write(geoTiff, path)
...
//       val geoTiff = SingleBandGeoTiff.compressed(geoTiffPath("econic_zlib_tiled_bandint.tif"))
...
//       extent should equal (geoTiff.extent)
// //      crs should equal (testCRS)
//       assertEqual(tile, geoTiff.tile)
//     }
...
class GeoTiffWriterSpec extends FunSpec
  ...
  describe ("writing GeoTiffs without errors and with correct tiles, crs and extent") {
    ...
    it ("should write floating point rasters correct") {
      ...
      GeoTiffWriter.write(path, t, e, testCRS)
    ...
    it ("should write floating point rasters with the default LatLng CRS correctly") {
      ...
      GeoTiffWriter.write(path, t, e, LatLng)
