...
class CursorSpec extends FunSpec with Matchers with TileBuilders {
  ...
  def checkAdded(r: Tile, cursor: Cursor, m: Movement, center: (Int, Int), expected: Seq[Int]) =
    checkSet(r, cursor.addedCells, cursor, m, center, expected)
  
  def checkRemoved(r: Tile, cursor: Cursor, m: Movement, center: (Int, Int), expected: Seq[Int]) =
    checkSet(r, cursor.removedCells, cursor, m, center, expected)

  def getAll(s: CellSet, r: Tile) = {
    val all = Set[Int]()
  ...
  describe("Cursor") {
    it("should have addedCells behave like allCells for a cursor that hasn't moved") {
      ...
      val r = createConsecutiveTile(10)
