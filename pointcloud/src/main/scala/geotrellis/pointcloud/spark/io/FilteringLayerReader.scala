...
import geotrellis.spark.io.{LayerQuery, BoundLayerQuery}
...
abstract class FilteringLayerReader[ID] extends LayerReader[ID] {
  ...
  ](id: ID, rasterQuery: LayerQuery[K, M], numPartitions: Int, indexFilterOnly: Boolean): RDD[(K, PointCloud)] with Metadata[M]

  def read[
    K: AvroRecordCodec: Boundable: JsonFormat: ClassTag,
    M: JsonFormat: GetComponent[?, Bounds[K]]
  ](id: ID, rasterQuery: LayerQuery[K, M], numPartitions: Int): RDD[(K, PointCloud)] with Metadata[M] =
    read(id, rasterQuery, numPartitions, false)

  def read[
    K: AvroRecordCodec: Boundable: JsonFormat: ClassTag,
    M: JsonFormat: GetComponent[?, Bounds[K]]
  ](id: ID, rasterQuery: LayerQuery[K, M]): RDD[(K, PointCloud)] with Metadata[M] =
  ...
  def query[
    K: AvroRecordCodec: Boundable: JsonFormat: ClassTag,
    M: JsonFormat: GetComponent[?, Bounds[K]]
  ](layerId: ID): BoundLayerQuery[K, M, RDD[(K, PointCloud)] with Metadata[M]] =
    new BoundLayerQuery(new LayerQuery, read(layerId, _))

  def query[
    K: AvroRecordCodec: Boundable: JsonFormat: ClassTag,
    M: JsonFormat: GetComponent[?, Bounds[K]]
  ](layerId: ID, numPartitions: Int): BoundLayerQuery[K, M, RDD[(K, PointCloud)] with Metadata[M]] =
