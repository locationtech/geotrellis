...
import geotrellis.pointcloud.spark.io.{LayerWriter => PCLayerWriter}
...
class FileLayerWriter(
    ...
    catalogPath: String
) extends PCLayerWriter[LayerId] with LazyLogging {

  protected def _write[
    K: AvroRecordCodec: JsonFormat: ClassTag,
    M: JsonFormat: GetComponent[?, Bounds[K]]
  ](layerId: LayerId, rdd: RDD[(K, PointCloud)] with Metadata[M], keyIndex: KeyIndex[K]): Unit = {
    ...
    val header =
      FileLayerHeader(
        ...
        valueClass = classTag[PointCloud].toString(),
        path = path
      )
