 ...
 * Spark application.
 */
class TestKryoSerializer(conf: SparkConf)
  ...
  private val userRegistrators = conf.get("spark.kryo.registrator", "")
    .split(',').map(_.trim)
    .filter(!_.isEmpty)
  private var classesToRegister = conf.get("spark.kryo.classesToRegister", "")
    .split(',').map(_.trim)
    .filter(!_.isEmpty)
  ...
  def newKryo(): Kryo = {
    ...
    for (cls <- TestKryoSerializer.toRegister) {
      kryo.register(cls)
    }
    for ((cls, ser) <- TestKryoSerializer.toRegisterSerializer) {
      kryo.register(cls, ser)
    }

    // For results returned by asJavaIterable. See JavaIterableWrapperSerializer.
    kryo.register(TestJavaIterableWrapperSerializer.wrapperClass, new TestJavaIterableWrapperSerializer)
  ...
  def addRegistration(newClass: String) = {
    classesToRegister = classesToRegister :+ newClass
  }

  override def newInstance(): SerializerInstance = {
    new TestKryoSerializerInstance(this)
  }
...
private[spark] class TestKryoSerializerInstance(ks: TestKryoSerializer) extends SerializerInstance {
  ...
  override def serialize[T: ClassTag](t: T): ByteBuffer = {
    ...
    try {
      ...
      case e: KryoException if e.getMessage.startsWith("java.lang.IllegalArgumentException: Class is not registered") 
                            && e.getMessage.contains("$anon") => {
        val msg = e.getMessage
        val omittedClass = msg.substring(msg.indexOf("(")+1, msg.indexOf(".class)"))
        println("\n\n***** =====> " ++omittedClass++" was registered implicitly.  \n\t\tBE SURE THAT YOU DO NOT WANT TO REGISTER THIS CLASS!\n")
        //val classLoader = Thread.currentThread.getContextClassLoader
        //val clss = Class.forName(omittedClass, true, classLoader)
        //println(kryo.getRegistration(classOf[Array[Boolean]]).toString)
        //println(kryo.getNextRegistrationId)
        releaseKryo(kryo)
        cachedKryo = null
        ks.addRegistration(omittedClass)
        kryo = borrowKryo()
        //println(kryo.getRegistration(classOf[Array[Boolean]]).toString)
        //println(kryo.getRegistration(clss).toString)
        //println(kryo.getNextRegistrationId)
        tryAgain = true
      }
    } finally {
  ...
  override def serializeStream(s: OutputStream): SerializationStream = {
    new TestKryoSerializationStream(this, s)
  }

  override def deserializeStream(s: InputStream): DeserializationStream = {
    new TestKryoDeserializationStream(this, s)
  }
...
private[spark]
class TestKryoSerializationStream(
    serInstance: TestKryoSerializerInstance,
...
private[serializer] object TestKryoSerializer {
  ...
  private val toRegisterSerializer = Map[Class[_], KryoClassSerializer[_]](
    classOf[RoaringBitmap] -> new KryoClassSerializer[RoaringBitmap]() {
      override def write(kryo: Kryo, output: KryoOutput, bitmap: RoaringBitmap): Unit = {
        bitmap.serialize(new TestKryoOutputObjectOutputBridge(kryo, output))
      }
      override def read(kryo: Kryo, input: KryoInput, cls: Class[RoaringBitmap]): RoaringBitmap = {
        val ret = new RoaringBitmap
        ret.deserialize(new TestKryoInputObjectInputBridge(kryo, input))
        ret
      }
    }
  )
 ...
 */
private[spark] class TestKryoInputObjectInputBridge(
 ...
 */
private[spark] class TestKryoOutputObjectOutputBridge(
 ...
 */
private class TestJavaIterableWrapperSerializer
  extends com.esotericsoftware.kryo.Serializer[java.lang.Iterable[_]] {

  import TestJavaIterableWrapperSerializer._
...
private object TestJavaIterableWrapperSerializer {
  ...
  private val underlyingMethodOpt = {
    try Some(wrapperClass.getDeclaredMethod("underlying")) catch {
      case e: Exception =>
        println("Failed to find the underlying field in " + wrapperClass + " " + e.getMessage)
        None
    }
  }
}
...
// import com.esotericsoftware.kryo.util.Util._
// import org.apache.spark.serializer._

// object TestKryo {
//   def chooseAction[R: ClassTag, T](strictAct: () => T, looseAct: () => T): T = {
//     try {
//       print("Trying strict serializer ... <<" ++ implicitly[ClassTag[R]].toString ++ ">>")
//       strictAct()
//     } catch {
//       case illarg: KryoException => {
//         val msg = illarg.getMessage
//         println("(1) " ++ msg)
//         val offendingClassName = msg.substring(msg.indexOf("(")+1, msg.indexOf(".class)"))
//         if (offendingClassName.contains("$anon")) {
//           println("Failing over to non-strict registration <<" ++ offendingClassName)
//           looseAct()
//         } else {
//           print("Non-anonymous function/class encountered in registration failure")
//           throw illarg
//         }
//       }
...
//       case ccarg: ClassCastException => {
//         val msg = ccarg.getMessage
//         print("Class cast exception: " ++ msg)
//         println("Failing over to non-strict registration")
//         looseAct()
//       }

//       case e: Throwable => {
//         //print("Total fail: " ++ e.getMessage ++ " <<" ++ e.getClass.getName ++ ">>")
//         throw e
//       }
//     } /* finally {
//       println("")
...
//   with Serializable 
// {
//   // println("Building TestKryoSerializer")
//   // println(conf.toDebugString)
  
//   val strict = new KryoSerializer(conf)

//   val confNoReg = conf.clone.remove("spark.kryo.registrationRequired")
//   val loose = new KryoSerializer(confNoReg)

//   println("Strict serializer is strict? " ++ strict.newKryo.isRegistrationRequired.toString)
//   println("Loose serializer is strict? " ++ loose.newKryo.isRegistrationRequired.toString)

//   def newInstance(): SerializerInstance = {
//     val strictInst = strict.newInstance
//     val looseInst = loose.newInstance

//     new TestKryoSerializerInstance(strictInst, looseInst)
//   }

// }

// class TestKryoSerializerInstance(
//   strict: SerializerInstance, 
//   loose: SerializerInstance) extends SerializerInstance {
  
//   override def serialize[T: ClassTag](t: T): ByteBuffer = {
//     println("TestKryoSerializerInstance.serialize[" ++ implicitly[ClassTag[T]].toString ++ "]")
//     TestKryo.chooseAction[T,ByteBuffer]({ () => strict.serialize(t) }, 
//                              { () => loose.serialize(t) })
//   }

//   def deserialize[T: ClassTag](bytes: ByteBuffer): T = {
//     println("TestKryoSerializerInstance.deserialize(1)[" ++ implicitly[ClassTag[T]].toString ++ "]")
//     TestKryo.chooseAction[T,T]({ () => strict.deserialize(bytes) }, { () => loose.deserialize(bytes) })
//   }

//   def deserialize[T: ClassTag](bytes: ByteBuffer, loader: ClassLoader): T = {
//     println("TestKryoSerializerInstance.deserialize(2)[" ++ implicitly[ClassTag[T]].toString ++ "]")
//     TestKryo.chooseAction[T,T]({ () => strict.deserialize(bytes, loader) }, { () => loose.deserialize(bytes, loader) })
//   }

//   def serializeStream(s: OutputStream): SerializationStream = { 
//     println("TestKryoSerializerInstance.serializeStream")
//     new TestKryoSerializationStream(strict.serializeStream(s), loose.serializeStream(s))
//   }

//   def deserializeStream(s: InputStream): DeserializationStream = {
//     println("TestKryoSerializerInstance.deserializeStream")
//     new TestKryoDeserializationStream(strict.deserializeStream(s), loose.deserializeStream(s))
//   }
// }

// class TestKryoSerializationStream(
//   strict: SerializationStream,
//   loose: SerializationStream) extends SerializationStream {
  
//   override def writeObject[T: ClassTag](t: T): SerializationStream = {
//     println("TestKryoSerializationStream.writeObject[" ++ implicitly[ClassTag[T]].toString ++ "]")
//     TestKryo.chooseAction[T,SerializationStream]({ () => strict.writeObject[T](t) }, { () => loose.writeObject[T](t) })
//   }

//   /** Writes the object representing the key of a key-value pair. */
//   override def writeKey[T: ClassTag](key: T): SerializationStream = {
//     println("TestKryoSerializationStream.writeKey[" ++ implicitly[ClassTag[T]].toString ++ "]")
//     TestKryo.chooseAction[T,SerializationStream]({ () => strict.writeKey[T](key) }, { () => loose.writeKey[T](key) })
//   }

//   /** Writes the object representing the value of a key-value pair. */
//   override def writeValue[T: ClassTag](value: T): SerializationStream = {
//     println("TestKryoSerializationStream.writeValue[" ++ implicitly[ClassTag[T]].toString ++ "]")
//     TestKryo.chooseAction[T,SerializationStream]({ () => strict.writeValue[T](value) }, { () => loose.writeValue[T](value) })
//   }

//   def flush(): Unit = {
//     println("TestKryoSerializationStream.flush")
//     strict.flush
//     loose.flush
//   }

//   def close(): Unit = {
//     println("TestKryoSerializationStream.close")
//     Try(strict.close)
//     Try(loose.close)
//   }
// }
...
// class TestKryoDeserializationStream(
//   strict: DeserializationStream,
//   loose: DeserializationStream) extends DeserializationStream {

//   /** The most general-purpose method to read an object. */
//   override def readObject[T: ClassTag](): T = {
//     println("TestKryoDeserializationStream.readObject[" ++ implicitly[ClassTag[T]].toString ++ "]")
//   TestKryo.chooseAction[T,T]({ () => strict.readObject[T]() }, { () => loose.readObject[T]() })
//   }

//   def close(): Unit = {
//     println("TestKryoDeserializationStream.close")
//     Try(strict.close)
//     Try(loose.close)
//   }
// }
