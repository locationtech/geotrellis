...
/**
  ...
  *
  * @param r           Tile to vizualize as an image
  * @param colorRamp   Colors to select from
  */
object SimpleRenderPng {
  def apply(r: Op[Tile], colors: Op[Array[Int]])(implicit d: DI): Op[Png] =
    r.flatMap { r =>
  ...
  def apply(
    r: Op[Tile], 
    colorRamp: Op[ColorRamp] = ColorRamps.HeatmapBlueToYellowToRedSpectrum
  ): Op[Png] =
