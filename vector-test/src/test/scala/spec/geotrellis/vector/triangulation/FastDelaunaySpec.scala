...
class FastDelaunaySpec extends FunSpec with Matchers {

  val numpts = 2000

  def randomPoint(extent: Extent) : LightPoint = {
    def randInRange (low : Double, high : Double) : Double = {
      val x = Random.nextDouble
      low * (1-x) + high * x
    }
    LightPoint(randInRange(extent.xmin, extent.xmax), randInRange(extent.ymin, extent.ymax))
  }
  ...
  describe("FastDelaunay Triangulation") {
    it("should have a convex boundary") {
      ...
      val dt = FastDelaunay(pts)

      def boundingEdgeIsConvex(e: HalfEdge[Int, LightPoint]) = {
