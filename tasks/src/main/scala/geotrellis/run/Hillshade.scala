...
object HillshadeTest {
  def main(args:Array[String]) {
    ...
    val i = io.LoadFile("/var/geotrellis/stroud/elevation30m-20110607.arg32")
    server.run(i)

    val start = System.currentTimeMillis
    println("hillshade")
    val h = focal.Hillshade(io.LoadFile("/var/geotrellis/stroud/elevation30m-20110607.arg32"), 45.0, 20.0)
    val grayscale = for (i <- 0 until 255) yield (i, i * (256 * 256) + i * 256 + i)
    
    val png = io.WritePNGFile(h,"/tmp/hillshade.png", Literal(grayscale.toArray), -1, true)
    server.run(png)
