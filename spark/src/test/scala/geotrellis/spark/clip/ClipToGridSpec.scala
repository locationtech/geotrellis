...
class ClipToGridSpec extends AnyFunSpec with TestEnvironment {
  describe("ClipToGrid") {
    ...
    it("should clip a point") {
      val p = Point(0, -10.0)
      val rdd = sc.parallelize(Array[Point](p).toIndexedSeq)
    ...
    it("should clip a multipoint") {
      ...
      val rdd = sc.parallelize(Array[MultiPoint](MultiPoint(p1, p2, p3)).toIndexedSeq)
    ...
    it("should clip a line") {
      ...
      val rdd = sc.parallelize(Array[LineString](line).toIndexedSeq)
    ...
    it("should clip a polygon") {
      ...
      val rdd = sc.parallelize(Array[Polygon](poly).toIndexedSeq)
      val actual = ClipToGrid(rdd, layoutDefinition)
