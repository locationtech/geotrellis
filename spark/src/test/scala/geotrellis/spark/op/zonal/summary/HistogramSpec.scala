...
class HistogramSpec extends FunSpec
  ...
  describe("Histogram Zonal Summary Operation") {
    ifCanRunSpark {
      ...
      it("should compute Histogram with all ones and whole extent") {
        val ones = sc.hadoopRasterRDD(allOnes.path)
        val polygon = extent.toPolygon

        val result = ones.zonalHistogram(polygon).collect

        result.size should equal (1)
        val histogram = result.head

        histogram.getItemCount(1) should equal (rddTotal)
      }
    }
  }
}
