...
trait CoordinateGridTimeKeyTests { self: PersistenceSpec[GridTimeKey, Tile, RasterMetadata[GridTimeKey]] =>
      ...
      it("query disjunction on space") {
        ...
        val expected = {
          for {
            ...
            time <- dates
          } yield GridTimeKey(col, row, time)
        }
