...
trait AndTileRDDMethods[K] extends TileRDDMethods[K] {
  ...
  /** And the values of each cell in each raster.  */
  def localAnd(others: Iterable[RDD[(K, Tile)]]): RDD[(K, Tile)] = localAnd(others, None)
  def localAnd(others: Iterable[RDD[(K, Tile)]], partitioner: Option[Partitioner]): RDD[(K, Tile)] =
    self.combineValues(others, partitioner){ And.apply }

  /** And the values of each cell in each raster. */
  def &(others: Iterable[RDD[(K, Tile)]]): RDD[(K, Tile)] = localAnd(others, None)
}
