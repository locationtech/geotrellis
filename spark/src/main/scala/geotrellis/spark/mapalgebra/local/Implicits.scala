...
trait Implicits {
  ...
  implicit class withLocalTileRDDSeqMethods[K](val self: Iterable[RDD[(K, Tile)]])
  ...
  implicit class withLocalTileCollectionSeqMethods[K](val self: Iterable[Seq[(K, Tile)]]) extends LocalTileCollectionSeqMethods[K]
}