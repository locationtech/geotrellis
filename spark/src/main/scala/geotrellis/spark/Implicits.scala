...
trait Implicits
  ...
  implicit class withCellGridLayoutRDDMethods[K: SpatialComponent: ClassTag, V <: CellGrid[Int], M: GetComponent[_, LayoutDefinition]](val self: RDD[(K, V)] with Metadata[M])
      extends CellGridLayoutRDDMethods[K, V, M]

  implicit class withProjectedExtentRDDMethods[K: Component[_, ProjectedExtent], V <: CellGrid[Int]](val rdd: RDD[(K, V)]) {
    def toRasters: RDD[(K, Raster[V])] =
      rdd.mapPartitions({ partition =>
  ...
  implicit class withTileProjectedExtentRDDMethods[K: Component[_, ProjectedExtent]: Component[_, CRS]](val rdd: RDD[(K, Tile)]) {
    def toGeoTiffs(
      tags: Tags = Tags.empty,
      options: GeoTiffOptions = GeoTiffOptions.DEFAULT
    ): RDD[(K, SinglebandGeoTiff)] =
  ...
  implicit class withMultibandTileProjectedExtentRDDMethods[K: Component[_, ProjectedExtent]: Component[_, CRS]](val rdd: RDD[(K, MultibandTile)]) {
    def toGeoTiffs(
      tags: Tags = Tags.empty,
      options: GeoTiffOptions = GeoTiffOptions.DEFAULT
    ): RDD[(K, MultibandGeoTiff)] =
  ...
  implicit class withProjectedExtentTemporalTilerKeyMethods[K: Component[_, ProjectedExtent]: Component[_, TemporalKey]](val self: K) extends TilerKeyMethods[K, SpaceTimeKey] {
  ...
  implicit class withProjectedExtentTilerKeyMethods[K: Component[_, ProjectedExtent]](val self: K) extends TilerKeyMethods[K, SpatialKey] {
    def extent = self.getComponent[ProjectedExtent].extent
