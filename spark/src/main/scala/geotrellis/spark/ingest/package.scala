...
package object ingest {
  ...
  implicit class withCollectMetadataMethods[K1, V <: CellGrid](rdd: RDD[(K1, V)]) extends Serializable {
    def collectMetaData[K2: SpatialComponent: Boundable](crs: CRS, layoutScheme: LayoutScheme)(implicit ev: K1 => TilerKeyMethods[K1, K2]): (Int, RasterMetaData[K2]) = {
      RasterMetaData.fromRdd(rdd, crs, layoutScheme)
    }

    def collectMetaData[K2: SpatialComponent: Boundable](crs: CRS, layout: LayoutDefinition)(implicit ev: K1 => TilerKeyMethods[K1, K2]): RasterMetaData[K2] = {
