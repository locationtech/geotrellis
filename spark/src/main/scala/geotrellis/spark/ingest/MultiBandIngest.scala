...
object MultiBandIngest{
  ...
  (sourceTiles: RDD[(T, MultiBandTile)], destCRS: CRS, layoutScheme: LayoutScheme, pyramid: Boolean = false, cacheLevel: StorageLevel = StorageLevel.NONE)
    ...
    (implicit tiler: Tiler[T, K, MultiBandTile]): Unit =
  {
    ...
    val tiledRdd = tiler(sourceTiles, rasterMetaData).cache()
