...
class TileRDDReprojectMethods[
  ...
  def reproject(destCrs: CRS, layoutScheme: LayoutScheme, options: Options)(
  ...
  def reproject(destCrs: CRS, layoutScheme: LayoutScheme)(
    implicit ev: Raster[V] => RasterRasterizeReprojectMethods[V]
  ): (Int, RDD[(K, V)] with Metadata[TileLayerMetadata[K]]) =
    reproject(destCrs, layoutScheme, Options.DEFAULT)

  def reproject(zoomedLayoutScheme: ZoomedLayoutScheme, options: Options)(
    implicit ev: Raster[V] => RasterRasterizeReprojectMethods[V]
  ): (Int, RDD[(K, V)] with Metadata[TileLayerMetadata[K]]) =
    reproject(zoomedLayoutScheme.crs, zoomedLayoutScheme, options)

  def reproject(zoomedLayoutScheme: ZoomedLayoutScheme)(
    implicit ev: Raster[V] => RasterRasterizeReprojectMethods[V]
  ): (Int, RDD[(K, V)] with Metadata[TileLayerMetadata[K]]) =
    reproject(zoomedLayoutScheme, Options.DEFAULT)

  def reproject(zoomedLayoutScheme: ZoomedLayoutScheme, bufferSize: Int, options: Options)(
    implicit ev: Raster[V] => RasterRasterizeReprojectMethods[V]
  ): (Int, RDD[(K, V)] with Metadata[TileLayerMetadata[K]]) =
    reproject(zoomedLayoutScheme.crs, zoomedLayoutScheme, bufferSize, options)

  def reproject(zoomedLayoutScheme: ZoomedLayoutScheme, bufferSize: Int)(
    implicit ev: Raster[V] => RasterRasterizeReprojectMethods[V]
  ): (Int, RDD[(K, V)] with Metadata[TileLayerMetadata[K]]) =
    reproject(zoomedLayoutScheme, bufferSize, Options.DEFAULT)

  def reproject(destCrs: CRS, layoutScheme: LayoutScheme, bufferSize: Int, options: Options)(
    implicit ev: Raster[V] => RasterRasterizeReprojectMethods[V]
  ): (Int, RDD[(K, V)] with Metadata[TileLayerMetadata[K]]) =
    TileRDDReproject(self, destCrs, Left(layoutScheme), bufferSize, options)

  def reproject(destCrs: CRS, layoutScheme: LayoutScheme, bufferSize: Int)(
    implicit ev: Raster[V] => RasterRasterizeReprojectMethods[V]
  ): (Int, RDD[(K, V)] with Metadata[TileLayerMetadata[K]]) =
    reproject(destCrs, layoutScheme, bufferSize, Options.DEFAULT)

  def reproject(destCrs: CRS, layoutDefinition: LayoutDefinition, bufferSize: Int, options: Options)(
    implicit ev: Raster[V] => RasterRasterizeReprojectMethods[V]
  ): (Int, RDD[(K, V)] with Metadata[TileLayerMetadata[K]]) =
    TileRDDReproject(self, destCrs, Right(layoutDefinition), bufferSize, options)

  def reproject(destCrs: CRS, layoutDefinition: LayoutDefinition, bufferSize: Int)(
    implicit ev: Raster[V] => RasterRasterizeReprojectMethods[V]
  ): (Int, RDD[(K, V)] with Metadata[TileLayerMetadata[K]]) =
    TileRDDReproject(self, destCrs, Right(layoutDefinition), bufferSize, Options.DEFAULT)

  def reproject(destCrs: CRS, layoutDefinition: LayoutDefinition, options: Options)(
    implicit ev: Raster[V] => RasterRasterizeReprojectMethods[V]
  ): (Int, RDD[(K, V)] with Metadata[TileLayerMetadata[K]]) =
    TileRDDReproject(self, destCrs, Right(layoutDefinition), options)

  def reproject(destCrs: CRS, layoutDefinition: LayoutDefinition)(
    implicit ev: Raster[V] => RasterRasterizeReprojectMethods[V]
  ): (Int, RDD[(K, V)] with Metadata[TileLayerMetadata[K]]) =
