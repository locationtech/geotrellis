...
object HilbertGridKeyIndex {
  def apply(minKey: GridKey, maxKey: GridKey, spatialResolution: Int): HilbertGridKeyIndex =
    apply(new KeyBounds(minKey, maxKey), spatialResolution)

  def apply(keyBounds: KeyBounds[GridKey], spatialResolution: Int): HilbertGridKeyIndex =
    apply(keyBounds, spatialResolution, spatialResolution)

  def apply(keyBounds: KeyBounds[GridKey], xResolution: Int, yResolution: Int): HilbertGridKeyIndex =
    new HilbertGridKeyIndex(keyBounds, xResolution, yResolution)
}

class HilbertGridKeyIndex(val keyBounds: KeyBounds[GridKey], val xResolution: Int, val yResolution: Int) extends KeyIndex[GridKey] {
