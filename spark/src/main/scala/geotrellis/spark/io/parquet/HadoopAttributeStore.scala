...
class ParqueteAttributeStore(val rootPath: Path)(implicit sc: SparkContext) extends BlobLayerAttributeStore {
  ...
  val p = ss.read.parquet(attributePath.getName)
  p.map { t => t.getAs[Long]("key") }
