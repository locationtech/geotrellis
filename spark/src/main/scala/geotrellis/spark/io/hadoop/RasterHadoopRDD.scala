...
//   def apply[K: HadoopWritable: ClassTag](path: String, sc: SparkContext, filters: FilterSet[K]): RasterRDD[K] =
//     apply(new Path(path), sc, filters)

//   def apply[K: HadoopWritable: Ordering: ClassTag](path: Path, sc: SparkContext): RasterRDD[K] =
//     apply(path, sc, FilterSet.EMPTY[K])

//   def apply[K: HadoopWritable: ClassTag](path: Path, sc: SparkContext, filters: FilterSet[K]): RasterRDD[K] = {
