...
object IterativeViewshed {
  ...
  def apply[K: (? => SpatialKey), V: (? => Tile)](
    ...
    ps.foreach({ p => require(p.length == 5) })
    ...
    val info: Seq[(SpatialKey, (Int, Int, Int, Double, Double, Double))] = { // inner tuple: index, col, row, z, angle, fov
      val fn = pointInfo(md)_
      ps.zipWithIndex.map(fn)
    }

    val _pointsByKey: Map[SpatialKey, Seq[(Int, Int, Int, Double, Double, Double)]] = // value: index, col, row, z, angle, fov
      info
        .groupBy(_._1)
    ...
    val _pointsByIndex: Map[Int, (SpatialKey, Int, Int, Double, Double)] = // value: key, col, angle, fov
      info
        .groupBy(_._2._1)
        .mapValues({ list => list.map({ case (key, (index, col, row, z, angle, fov)) =>
          (key, col, row, angle, fov) }) })
      ...
      elevation
        .flatMap({ case (k, v) =>
          ...
          pointsByKey.value.get(key) match {
            case Some(list) =>
              list.map({ case (index: Int, col: Int, row: Int, z: Double, _, _) =>
    ...
    var sheds: RDD[(K, V, MutableArrayTile)] = elevation.map({ case (k, v) =>
      ...
      pointsByKey.value.get(key) match {
        case Some(list) =>
          list.foreach({ case (index: Int, col: Int, row: Int, z: Double, angle: Double, fov: Double) =>
        ...
        changes.value.get(key) match {
          case Some(localChanges: Seq[(Int, From, Ray)]) => { // sequence of <index, from, ray> triples for this key
            ...
            indexed.foreach({ case (index, list) => // for all <from, ray> pairs generated by this point (this index)
              val (pointKey, col, row, angle, fov) = pointsByIndex.value.getOrElse(index, throw new Exception)
