...
package object ingest {
  ...
  implicit class MetaDataWrapper[T](rdd: RDD[(T, Tile)])(implicit _projectedExtent: SimpleLens[T, ProjectedExtent]) {
    def createMetaData(layerName: String, layoutScheme: LayoutScheme): LayerMetaData = {
  ...
  implicit class MosaicWrapper[T, K: ClassTag](rdd: RDD[(T, Tile)])(implicit _extent: SimpleLens[T, Extent]) {
    def mosaic(rasterMetaData: RasterMetaData)(getExtent: T => Extent)(createKey: (T, SpatialKey) => K)(implicit _spatialKey: SimpleLens[K, SpatialKey]): RasterRDD[K] = {
      ...
      def combineTiles1(tile: MutableArrayTile, tup: (K, Extent, Tile)): MutableArrayTile = {
        ...
        val metaData = bcMetaData.value
        tile.merge(metaData.transform.gridToMap(key |-> _spatialKey get), extent, prevTile)
      }
