...
object VoronoiDiagram {
  ...
  def cellExtentIntersection(cell: Seq[CellBound], extent: Extent) = {
  ...
  private def polygonalCell(het: HalfEdgeTable, verts: Int => Coordinate, extent: Extent, debug: Boolean)(incidentEdge: Int): Option[Polygon] = {
    val cell = cellBoundsNew(het, verts, extent)(incidentEdge)
    if (debug) {
      println(s"Cell boundary: $cell")
    }
    ...
    // accum += accum.head
    // val result = Polygon(accum.map(Point.jtsCoord2Point(_)))
    val cei = cellExtentIntersection(cell, extent)

    // println(s"cellExtentIntersection: ${cei}")
    // println(s"polygonalCell: $result")
...
class VoronoiDiagram(val dt: DelaunayTriangulation, val extent: Extent) {
  ...
  def voronoiCell(i: Int): Option[Polygon] = {
    polygonalCell(dt.halfEdgeTable, pointSet.getCoordinate(_), extent, dt.debug)(dt.halfEdgeTable.edgeIncidentTo(i))
  }
