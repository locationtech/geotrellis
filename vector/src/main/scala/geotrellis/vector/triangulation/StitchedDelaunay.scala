...
object StitchedDelaunay {
  ...
  def apply(neighbors: Map[Direction, (BoundaryDelaunay, Extent)], debug: Boolean = false): StitchedDelaunay = {
    ...
    val halfEdgeTable = new HalfEdgeTable(2 * (3 * vertCount - 6))
    ...
    val boundaries = neighbors.map{ case (dir, (bdt, _)) => {
      ...
      val edgeoffset = halfEdgeTable.appendTable(bdt.halfEdgeTable, vtrans(dir)(_))
    ...
    val overlayTris = new TriangleMap(halfEdgeTable)
    val stitcher = new DelaunayStitcher(allPoints, halfEdgeTable)
    ...
    new StitchedDelaunay(allPoints.getCoordinate(_), halfEdgeTable, allPoints, overlayTris)
  }
  ...
  def apply(center: DelaunayTriangulation, neighbors: Map[Direction, (BoundaryDelaunay, Extent)], debug: Boolean): StitchedDelaunay = {
    ...
    val boundaries = neighbors.map{ case (dir, (bdt, _)) => {
      val reindex = vtrans(dir).apply(_)
      if (dir == Center) {
        val edgeoffset = halfEdgeTable.appendTable(center.halfEdgeTable, vtrans(dir)(_))
