  ...
  def getMetadata(url: URL, epsg: String = "EPSG:3785"): Option[Metadata] =
    accepts(url, epsg) match {
      ...
      case true => {
        ...
        Some(Metadata(coverage.getEnvelope.asInstanceOf[GeneralEnvelope], pixelSize, pixels, bands, cellType, nodata))
      }
    }
  ...
  def readRaster(path: String): (Tile, RasterExtent) = {
    ...
    val readState =
      if (cellType.isFloatingPoint) {
        new GeoTiffDoubleReadState(path, rasterExtent, rasterExtent, cellType, reader)
      } else {
        new GeoTiffIntReadState(path, rasterExtent, rasterExtent, cellType, reader)
      }
