...
/*
class RasterToGridCoverage2D_FloatMultibandSpec extends RasterToGridCoverage2DSpec[MultibandTile] {
  val cellType = FloatCellType
  val bands = Array(
    ArrayTile.empty(cellType, 512, 512),
    ArrayTile.empty(cellType, 512, 512),
  ...
  val crs = None
  val nd = None
  bands(0).setDouble(1, 1, 33.0); bands(1).setDouble(1, 1, 42.0); bands(2).setDouble(2, 2, 107.0)
  val gridCoverage = MultibandTileRasterToGridCoverage2D(Raster(tile, extent), crs)

  describe("Copying of Data") {

    it("should leave blank pixels blank") {
      getPixel(0,0).toList should be (List(0.0, 0.0, 0.0))
    }

    it("should preserve pixels that have been set") {
      getPixel(1, 1).toList should be (List(33.0, 42.0, 0.0))
    }

    it("should preserve other pixels that have been set") {
      getPixel(2, 2).toList should be (List(0.0, 0.0, 107.0))
    }
  }
}

class RasterToGridCoverage2D_DoubleMultibandSpec extends RasterToGridCoverage2DSpec[MultibandTile] {
  val cellType = DoubleCellType
  val bands = Array(
    ArrayTile.empty(cellType, 512, 512),
    ArrayTile.empty(cellType, 512, 512),
