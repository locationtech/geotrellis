...
trait Implicits extends merge.Implicits
  ...
  implicit class withCellGridLayoutCollectionMethods[K: SpatialComponent, V <: CellGrid[Int], M: GetComponent[_, LayoutDefinition]](val self: Seq[(K, V)] with Metadata[M])
