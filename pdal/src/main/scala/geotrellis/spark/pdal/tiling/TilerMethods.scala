...
package geotrellis.spark.pdal.tiling
...
class TilerMethods[K](val self: RDD[(K, PackedPoints)]) extends MethodExtensions[RDD[(K, PackedPoints)]] {
  def tileToLayout[K2: SpatialComponent: ClassTag](layoutDefinition: LayoutDefinition)
                                                  (implicit ev: K => TilerKeyMethods[K, K2]): RDD[(K2, PackedPoints)] =
    CutPackedPoints[K, K2](self, layoutDefinition)
