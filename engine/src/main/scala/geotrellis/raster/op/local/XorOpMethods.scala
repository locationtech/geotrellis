...
trait XorOpMethods[+Repr <: RasterSource] { self: Repr =>
  ...
  def localXor(i: Int): RasterSource = self.mapTile(Xor(_, i))
  /** Xor a constant Int value to each cell. */
  def ^(i: Int): RasterSource = localXor(i)
  /** Xor a constant Int value to each cell. */
  def ^:(i: Int): RasterSource = localXor(i)
  /** Xor the values of each cell in each raster.  */
  def localXor(rs: RasterSource): RasterSource = self.combineTile(rs)(Xor(_, _))
  /** Xor the values of each cell in each raster. */
  def ^(rs: RasterSource): RasterSource = localXor(rs)
  /** Xor the values of each cell in each raster. */
  def localXor(rss: Seq[RasterSource]): RasterSource = self.combineTile(rss)(Xor(_))
  /** Xor the values of each cell in each raster. */
