...
trait GreaterOrEqualOpMethods[+Repr <: RasterSource] { self: Repr =>
   ...
   * integer, else 0.
   */
  def localGreaterOrEqual(i: Int): RasterSource = self.map(GreaterOrEqual(_, i))
  /**
   * Returns a Raster with data of TypeBit, where cell values equal 1 if
   * the corresponding cell value of the input raster is greater than or equal to the input
   * integer, else 0.
   */
  def localGreaterOrEqualRightAssociative(i: Int): RasterSource = self.map(GreaterOrEqual(i, _))
  /**
   * Returns a Raster with data of TypeBit, where cell values equal 1 if
   * the corresponding cell value of the input raster is greater than or equal to the input
    * integer, else 0.
   ...
   * double, else 0.
   */
  def localGreaterOrEqual(d: Double): RasterSource = self.map(GreaterOrEqual(_, d))
  /**
   * Returns a Raster with data of TypeBit, where cell values equal 1 if
   * the corresponding cell value of the input raster is greater than or equal to the input
   * double, else 0.
   */
  def localGreaterOrEqualRightAssociative(d: Double): RasterSource = self.map(GreaterOrEqual(d, _))
  /**
   * Returns a Raster with data of TypeBit, where cell values equal 1 if
   * the corresponding cell value of the input raster is greater than or equal to the input
   * double, else 0.
   ...
   */
  def localGreaterOrEqual(rs: RasterSource): RasterSource = self.combine(rs)(GreaterOrEqual(_, _))
  /**
   * Returns a Raster with data of TypeBit, where cell values equal 1 if
   * the corresponding cell valued of the rasters are greater than or equal to the next raster, else 0.
   */
