...
case class StepAggregator[T](engineContext:EngineContext, 
  ...
  val handler = new ResultHandler(engineContext,client,pos)
  ...
  override def preStart {
    for (i <- 0 until args.length) {
      args(i) match {
        case lit:Literal[_] =>
          val v = lit.value
          results(i) = Some(Complete(v,History.literal(v,engineContext.externalId)))
        case op:Operation[_] =>
          engineContext.engineRef ! RunOperation(op, i, self)
        case value =>
