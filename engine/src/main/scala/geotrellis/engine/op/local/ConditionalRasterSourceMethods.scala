...
trait ConditionalRasterSourceMethods extends RasterSourceMethods {
  ...
  def localIf(cond: Int => Boolean, trueValue: Int): RasterSource =
    rasterSource.mapTile(IfCell(_, cond, trueValue), "IfCell")
  ...
  def localIf(cond: Int => Boolean, trueValue: Int, falseValue: Int): RasterSource =
    rasterSource.mapTile(IfCell(_, cond, trueValue, falseValue), "IfCell")
  ...
  def localIf(rs: RasterSource, cond: (Int, Int)=>Boolean, trueValue: Int): RasterSource =
    rasterSource.combineTile(rs, "BinaryIfCell")(IfCell(_, _, cond, trueValue))
  ...
  def localIf(rs: RasterSource, cond: (Int, Int)=>Boolean, trueValue: Int, falseValue: Int): RasterSource =
    rasterSource.combineTile(rs, "BinaryIfElseCell")(IfCell(_, _, cond, trueValue, falseValue))
