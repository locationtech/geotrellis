...
In the following, we'll be providing some Scala code samples that make use of the GeoTrellis library.  This requires having a running Scala instance with GeoTrellis set as a library dependency.  It is recommended that the user download and install SBT, the Scala Build Tool.  Once that is installed, it is recommended that you use `git` to clone the repository at `github.com/geotrellis/geotrellis-sbt-template repository`.  Once available, from the root directory of the cloned repo, MacOSX and Linux users may launch the sbt script contained therein.  This will start an SBT session.  Once SBT is started, it is advisable to issue the `console` command; this will start the Scala interpreter.  At this point, you should be able to issue the command `import geotrellis.vector._` without raising an error.
...
In either event, it will be necessary to install Spark in your local environment to run the code above.  Once that is done, you will need to clone the GeoTrellis repository from [Github](https://github.com/geotrellis/geotrellis).  From the root directory of that project, execute the provided `sbt` script.  Once SBT is loaded, the following sequence of commands can be executed:
```scala
  project util
  assembly
  project proj4
  ...
  set assemblyMergeStrategy in assembly := { case "META-INF/MANIFEST.MF" => MergeStrategy.discard ; case _ => MergeStrategy.first }
  assembly
```
This builds the required JAR files.  Now, again from the GeoTrellis source tree root directory, issue the command
```bash
  spark-shell --jars util/target/scala-2.10/geotrellis-util-assembly-[version].jar,proj4/target/scala-2.10/geotrellis-proj4-assembly-[version].jar,vector/target/scala-2.10/geotrellis-vector-assembly-[version].jar,raster/target/scala-2.10/geotrellis-raster-assembly-[version].jar,spark/target/scala-2.10/geotrellis-spark-assembly-[version].jar
```
