...
object Ingest {
  ...
  private def getPixelBounds(envelope: Envelope, zoom: Int, tileSize: Int) = {
    val (w, s, e, n) =
  ...
  def toRasterData(tile: GridCoverage, imageMeta: Metadata, zoom: Int, tileSize: Int): RasterData = {
    val pixelBounds = getPixelBounds(tile.getEnvelope(), zoom, tileSize)
    val (cols, rows) = (pixelBounds.width.toInt, pixelBounds.height.toInt)
    val rawDataBuff = tile.getRenderedImage().getData().getDataBuffer()
    val rd = imageMeta.rasterType match {
      case DataBuffer.TYPE_FLOAT => RasterData(rawDataBuff.asInstanceOf[DataBufferFloat].getData(), cols, rows)
      case DataBuffer.TYPE_INT   => RasterData(rawDataBuff.asInstanceOf[DataBufferInt].getData(), cols, rows)
