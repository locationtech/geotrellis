...
class S3GridKeySpec
  ...
  lazy val updater = new S3LayerUpdater(attributeStore, reader) { override def rddWriter = S3GridKeySpec.this.rddWriter }
  ...
  lazy val tiles = new S3TileReader[GridKey, Tile](attributeStore) { override val s3Client = new MockS3Client()  }
