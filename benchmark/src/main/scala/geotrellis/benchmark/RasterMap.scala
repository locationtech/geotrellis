...
//  @Param(Array("512"))
  ...
  def thisMap(r: Tile, f: Int => Int) = r map { i =>
      if(isNoData(i)) i
      else f(i)
    }
  ...
  override def setUp() {
    ...
    raster = ArrayTile(init(len)(Random.nextInt), size, size)

    doubleRaster = ArrayTile(init(len)(Random.nextDouble), size, size)
