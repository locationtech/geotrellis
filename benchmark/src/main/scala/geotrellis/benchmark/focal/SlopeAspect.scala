...
abstract class SlopeAspectCalculator[@specialized(Int,Double) D](zFactor:Double,
								  cellWidth:Double,
  ...
  def getSlopeAndAspect: (Double, Double) = {
    ...
    val e = east(0) + 2*east(1) + east(2)
    val w = west(0) + 2*west(1) + west(2)
    val n = west(0) + 2*base(0) + east(0)
    val s = west(2) + 2*base(2) + east(2)
...
protected case class AspectCalc(zFactor:Double, 
		       cellWidth:Double, 
...
protected case class SlopeCalc(zFactor:Double,
		      cellWidth:Double,
		      cellHeight:Double) extends SlopeAspectCalculator[Double](zFactor,cellWidth,cellHeight) {
  def getResult:Double = {
    if(isNoData(base(1))) return NODATA
