...
class GridKeyRenderOutput extends OutputPlugin[GridKey, Tile, RasterMetadata[GridKey]] {
  def name = "render"
  ...
  override def apply(
    id: LayerId,
    rdd: RDD[(GridKey, Tile)] with Metadata[RasterMetadata[GridKey]],
    ...
    val images =
      props("encoding").toLowerCase match {
        case "png" =>
          rdd.asInstanceOf[RDD[(GridKey, Tile)] with Metadata[RasterMetadata[GridKey]]].renderPng(parseClassifications(props.get("breaks")))
        case "geotiff" =>
          rdd.asInstanceOf[RDD[(GridKey, Tile)] with Metadata[RasterMetadata[GridKey]]].renderGeoTiff()
      }
